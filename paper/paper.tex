...
(3)~Two models vote Nein on 83--94\% of referenda at similar rates on progressive and conservative proposals (binomial $p < 0.0001$), a status-quo aversion rather than a left-right bias.
...
Two further results compound this instability: cross-linguistic consistency ranges from 50\% (Mistral) to 98\% (GPT-5.4), and two models (Grok, Mistral) vote Nein on 83--94\% of referenda at similar rates on progressive and conservative proposals (binomial $p < 0.0001$), indicating status-quo aversion rather than a left-right bias.
...
\citet{ceron2024beyond} found left-leaning outputs on environment and welfare but right-leaning ones on law and order, and argumentative pressure toward opposite preferences preserves 58--95\% of original stances~\citep{press2025framework}.
...
For the Volksabstimmungen experiment, we selected 9 flagship models representing the frontier of each major provider family (Table~\ref{tab:flagships}): four countries, two open-source, seven closed-source.
...
No structural variable predicts positioning: geographic origin ($p = 0.872$), open- vs.\ closed-source ($p = 0.870$), and temporal drift (12 pairs, 6 each way, $p = 1.000$) are all null.
The centroid sits nearest GLP and Die~Mitte, with individual models spanning SP to Die~Mitte.

The Smartvote agreement heatmap (Figure~\ref{fig:heatmap}) shows the expected left-to-right gradient: highest agreement with SP and Gr\"{u}ne, lowest with SVP, though 37 of 66 models deviate from strict monotonicity, most commonly by peaking at GLP.
...
Mistral is the extreme case: its Ja rate swings from 17\% in German to 82\% in Romansh, though 42\% of Romansh queries are refused (\S\ref{sec:refusal}), so the 82\% rests on approximately 28 responses and would fall to $\sim$48\% if refusals were Nein.
...
Two further instabilities compound this: for Mistral and Llama the query language changes the answer more than the content does, through processing differences that do not track the real R\"{o}stigraben; and two models vote Nein on 83--94\% of referenda at similar rates whether progressive or conservative, a status-quo aversion rather than a left-right bias.
...
The Smartvote tests have limited power to detect small structural effects (the drift sign test has $\sim$39\% power at $p = 0.75$ with $n = 12$ pairs).
